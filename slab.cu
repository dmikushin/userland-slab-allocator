...
namespace gpu
{
    __device__ size_t slab_pagesize;
    __device__ void* pool;
    __device__ int page_index = 0;
}
...
#define FIRST_FREE_SLOT(s) ((size_t) __clzll(__brevll(s)))
#define FREE_SLOTS(s) ((size_t) __popcll(s))
#if (defined(_MSC_VER) && defined(_WIN64)) || defined(__LP64__) || defined(__CUDACC_RTC__)
#define __PTR   "l"
#else
#define __PTR   "r"
#endif
#define __builtin_prefetch(addr, ...) asm("prefetch.global.L2 [%0];" :: __PTR(addr));
#define posix_memalign posix_memalign_device
__device__
inline __attribute__((always_inline))
int posix_memalign_device(void **memptr, size_t alignment, size_t size)
{
    *memptr = gpu::pool;
    return 0;
}
#define mmap mmap_device
__device__
inline __attribute__((always_inline))
void *mmap_device(void *addr, size_t length, int prot, int flags, int fd, off_t offset)
{
    return (char*)gpu::pool + gpu::page_index++ * gpu::slab_pagesize;
}
#define munmap munmap_device
__device__
inline __attribute__((always_inline))
int munmap_device(void *addr, size_t length)
{
    return 0;
}
#else
...
{
#if defined(__CUDA_ARCH__)
    using gpu::slab_pagesize;
#else
    using cpu::slab_pagesize;
#endif
    ...
    for (size_t head = 0; head < 3; ++head) {
        ...
        for (slab = heads[head]; slab != NULL; slab = slab->next) {
            const struct slab_header* slabPtr = slab;
#if !defined(__CUDA_ARCH__)
            cudaPointerAttributes attrs;
            cudaError err = cudaPointerGetAttributes(&attrs, slab);
            cudaMemcpyKind kind = cudaMemcpyHostToHost;
            if (!err && (attrs.memoryType == cudaMemoryTypeDevice))
                kind = cudaMemcpyDeviceToHost;
            struct slab_header slabHost;
            CUDA_ERR_CHECK(cudaMemcpy(&slabHost, slab, sizeof(slab_header), kind));
            slab = &slabHost;
#endif
            if (prev == NULL)
            ...
            if (slab->refcount == 0) {
                assert((uintptr_t) slabPtr % sch->slabsize == 0);
                ...
                if (sch->slabsize >= slab_pagesize)
                    ...
                    assert((uintptr_t) slabPtr % slab_pagesize == 0);
            }
...
void slab_init(struct slab_chain *const sch, const size_t itemsize)
{
    ...
    sch->slabsize = (size_t) 1 << (size_t) ceil(log2f(least_slabsize));
    sch->itemcount = 64;
...
void *slab_alloc(struct slab_chain *const sch)
{
    ...
    if (LIKELY(sch->partial != NULL)) {
        ...
        if (sch->slabsize <= slab_pagesize) {
            sch->partial = (slab_header*)mmap(NULL, sch->pages_per_alloc,
...
void slab_free(struct slab_chain *const sch, const void *const addr)
{
    ...
    struct slab_header *const slab = (slab_header*)
    ...
    if (UNLIKELY(slab->slots == SLOTS_ALL_ZERO)) {
        ...
        if (UNLIKELY(slab->refcount == 1 || (slab->refcount == 0 &&
            ...
            if (LIKELY(slab != sch->partial)) {
                if (slab->prev && LIKELY((slab->prev->next = slab->next) != NULL))
                    slab->next->prev = slab->prev;
            } else if (LIKELY((sch->partial = sch->partial->next) != NULL)) {
                sch->partial->prev = NULL;
            }

            char *const page = (char*)(UNLIKELY(slab->refcount != 0) ? slab : slab->page);
            ...
            for ( ; s.c != page_end; s.c += sch->slabsize) {
                if (UNLIKELY(s.s == sch->empty))
                    ...
                    continue;
                else if (s.s->prev && LIKELY((s.s->prev->next = s.s->next) != NULL))
...
__global__
void slab_alloc_kernel(struct slab_chain* s, void** ptr)
{
    *ptr = slab_alloc(s);
}

__global__
void slab_free_kernel(struct slab_chain* s, void* ptr)
{
    slab_free(s, ptr);
}

__global__
void slab_init_kernel(struct slab_chain *const sch, const size_t itemsize)
{
    slab_init(sch, itemsize);
}

__global__
void slab_destroy_kernel(const struct slab_chain *const sch)
{
    slab_destroy(sch);
}
...
int main(int argc, char* argv[])
{
    ...
    cpu::slab_pagesize = (size_t) sysconf(_SC_PAGESIZE);
    ...
    if ((string)argv[1] == "gpu")
    {
        ...
        for ( ; size >= 1; size *= 0.9)
        {
            status = cudaMalloc((void**)&cpu::pool, size);
        ...
        CUDA_ERR_CHECK(cudaMemcpyToSymbol(
            gpu::slab_pagesize, &cpu::slab_pagesize, sizeof(size_t)));

        // Align pool start to the size of page.
        if ((size_t)cpu::pool % cpu::slab_pagesize)
            cpu::pool = (void*)((char*)cpu::pool + 
                cpu::slab_pagesize - ((size_t)cpu::pool % cpu::slab_pagesize));

        // Make pool size to be power of two.
        size = pow(2, floor(log2f(size)));
    
        std::cout << "Device memory heap size: " << size << std::endl;

        CUDA_ERR_CHECK(cudaMemcpyToSymbol(
            gpu::pool, &cpu::pool, sizeof(void*)));

        struct slab_chain s;
        double *allocs[16 * 60];

        struct slab_chain* sDev = NULL;
        CUDA_ERR_CHECK(cudaMalloc(&sDev, sizeof(struct slab_chain)));

        slab_init_kernel<<<1, 1>>>(sDev, sizeof(double));
        CUDA_ERR_CHECK(cudaGetLastError());
        CUDA_ERR_CHECK(cudaDeviceSynchronize());

        void** ptr = NULL;
        CUDA_ERR_CHECK(cudaMalloc(&ptr, sizeof(void*)));

        for (size_t i = 0; i < sizeof(allocs) / sizeof(*allocs); i++) {
            slab_alloc_kernel<<<1, 1>>>(sDev, ptr);
            CUDA_ERR_CHECK(cudaGetLastError());
            CUDA_ERR_CHECK(cudaDeviceSynchronize());
            CUDA_ERR_CHECK(cudaMemcpy(&allocs[i], ptr, sizeof(void*),
                cudaMemcpyDeviceToHost));

            assert(allocs[i] != NULL);

            CUDA_ERR_CHECK(cudaMemcpy(&s, sDev, sizeof(struct slab_chain),
                cudaMemcpyDeviceToHost));
        ...
        for (ssize_t i = sizeof(allocs) / sizeof(*allocs) - 1; i >= 0; i--) {
            CUDA_ERR_CHECK(cudaMemcpy(ptr, &allocs[i], sizeof(void*),
                cudaMemcpyDeviceToHost));
            slab_free_kernel<<<1, 1>>>(sDev, allocs[i]);
            CUDA_ERR_CHECK(cudaGetLastError());
            CUDA_ERR_CHECK(cudaDeviceSynchronize());
